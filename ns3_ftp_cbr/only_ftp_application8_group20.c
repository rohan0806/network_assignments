...
main (int argc, char *argv[])
{
  ...
  uint16_t port = 2;   // Discard port (RFC 863)
  
  BulkSendHelper ftp("ns3::TcpSocketFactory", InetSocketAddress (i3i5.GetAddress (1), port));
  ftp.SetAttribute ("SendSize", UintegerValue (100));
  // Set the amount of data to send in bytes.  Zero is unlimited.
  ftp.SetAttribute ("MaxBytes", UintegerValue (0));
  ApplicationContainer ftpApp = ftp.Install (c.Get (0));
  ftpApp.Start (Seconds (1));
  ftpApp.Stop (Seconds (10)); 
  

  PacketSinkHelper sink3 ("ns3::TcpSocketFactory",
                          Address (InetSocketAddress (Ipv4Address::GetAny (), port)));
  ftpApp = sink3.Install (c.Get (5));
